	...
	if form.is_valid():
		...
		if List.objects.first() is None:
			...
			for number,face in enumerate(unknown_face_encoding, start=1):
				check_with_known_faces = face_recognition.compare_faces(known_faces_array, face) #this is just the compare function, I should make it so we can toggle the distance threshold
				face_distances = face_recognition.face_distance(known_faces_array, face)
				print("there are {} matches for face {}, with the closest distance being {}".format(sum(face_distances < 0.5),number,min(face_distances)))
				print2 = "there are {} matches for face {}, with the closest distance being {}".format(sum(face_distances < 0.5),number,min(face_distances))
				instance.print2 = print2

				if sum(face_distances < 0.5) > 0:
					first_match_index = list(face_distances).index(min(face_distances)) + 1
					print("i have identified person {}, and i recognise them \n ".format(first_match_index))
